...
class Strategy(analytics.Analytics, plot.Plot):
    ...
    def __init__(self, strategy_report_path = None, daily_report_df = None, trading_record_df = None):
        ...
        nav_list = [1] + list((daily_report_df['profit'].cumsum() + self.max_holding_money) / self.max_holding_money)
        nav_df = pd.DataFrame()
        nav_df['nav'] = nav_list
